...
HRESULT inventory::init()
{
	...
	for (int i = 0; i < 5; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			...
			_inventoryelement.x = 190 + i * 55;
			_inventoryelement.y = 240 + j * 60;
	...
	for (int i = 0; i < 2; i++)
	{
		for (int j = 0; j < 4; j++)
		{
			_inventoryelement._inventoryrect = RectMakeCenter(612 + i * 132, 240, 40, 40);
			if (j >= 1)
			{
				_inventoryelement._inventoryrect = RectMakeCenter(555 + i * 50, 260 + j * 50, 40, 40);
			}
			_inventoryelement._item = ITEMMANAGER->addItem("비어있음");
			_inventoryelement._item.setRect(_inventoryelement._inventoryrect);
			_vInven.push_back(_inventoryelement);
...
void inventory::cursormove()
{
	...
	{
		if (_cursorNumber > 0) 	_cursorNumber -= 4;
	...
	if (KEYMANAGER->isOnceKeyDown('D'))
	{
		if (!_storageOpen)
		{
			if (_cursorNumber < 28) _cursorNumber += 4;
		}
		else
		{
			if (_cursorNumber > 19)
			...
			else
			{
				if (_vInven[i]._item.getItemInfo().itemName == "비어있음")
				{
					if ((_cursorNumber == i) && KEYMANAGER->isOnceKeyDown('J'))
					{
						...
						//	_vInven.insert(_vInven.begin() + i, itemnameswap("비어있음"));
						_vTemp.pop_back();
						break;
					}
				}
