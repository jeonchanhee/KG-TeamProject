...
HRESULT PlayerHpbar::init(char * frontImage, char * backImage, float x, float y, int width, int height)
{
	_x = x;
	_y = y;
...
void PlayerHpbar::render()
{
	...
	_progressBarFront->render(CAMERAMANAGER->getCameraDC(), _rcProgress.left + 26, _y, 0, 0, _width, _progressBarBack->getHeight());
}
